  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS post
      CHANGING lt_input TYPE ZZT_ZZTFI_0003
               ...
               msg      TYPE bapi_msg.
    CLASS-METHODS post_cf
      IMPORTING s_str   TYPE string
      CHANGING lt_input TYPE ZZT_ZZTFI_0003
               flag     TYPE bapi_mtype
               msg      TYPE bapi_msg.
PROTECTED SECTION.
        ...
        LOOP AT lt_input INTO DATA(ls_input).
          CASE ls_input-koart.
            WHEN 'S'.
              ...
              <fs_item>-reference_document_item = ls_input-pk_line_no_1.
...
*              ENDIF.
              ...
              <fs_item>-reference3idby_business_partne = ls_input-reference3idbybusinesspartner.
              <fs_item>-reference1idby_business_partne = ls_input-reference1idbybusinesspartner.
              <fs_item>-reference2idby_business_partne = ls_input-reference2idbybusinesspartner.
              <fs_item>-account_assignment-cost_center = ls_input-costcenter.
            WHEN 'A'.
              APPEND INITIAL LINE TO <fs_entry>-debtor_item ASSIGNING FIELD-SYMBOL(<fs_ditem>).
              <fs_ditem>-reference_document_item = ls_input-pk_line_no_1.
              ...
              <fs_ditem>-amount_in_group_currency-currency_code = 'CNY'.
...
*              ENDIF.
              ...
              <fs_ditem>-reference3idby_business_partne = ls_input-reference3idbybusinesspartner.
              <fs_ditem>-reference1idby_business_partne = ls_input-reference1idbybusinesspartner.
              <fs_ditem>-reference2idby_business_partne = ls_input-reference2idbybusinesspartner.
            WHEN 'K'.
              APPEND INITIAL LINE TO <fs_entry>-creditor_item ASSIGNING FIELD-SYMBOL(<fs_citem>).
              <fs_citem>-reference_document_item = ls_input-pk_line_no_1.
              <fs_citem>-creditor = ls_input-vendor.
              <fs_citem>-debit_credit_code = ls_input-debitcreditcode.
              <fs_citem>-altv_recncln_accts-content = ls_input-altvrecnclnaccts.
              <fs_citem>-amount_in_transaction_currency-currency_code = ls_input-transactioncurrency.
              <fs_citem>-amount_in_transaction_currency-content = ls_input-amountintransactioncurrency.
              <fs_citem>-amount_in_group_currency-currency_code = 'CNY'.
              <fs_citem>-amount_in_group_currency-content = ls_input-amountingroupcurrency.
*              IF <fs_citem>-debit_credit_code = 'H'.
*                <fs_citem>-amount_in_transaction_currency-content = - abs( ls_input-amountintransactioncurrency ).
*                <fs_citem>-amount_in_group_currency-content = - abs( ls_input-amountingroupcurrency ).
*              ENDIF.
              <fs_citem>-assignment_reference = ls_input-assignmentreference.
              <fs_citem>-document_item_text = ls_input-documentitemtext.
              <fs_citem>-reference3idby_business_partne = ls_input-reference3idbybusinesspartner.
              <fs_citem>-reference1idby_business_partne = ls_input-reference1idbybusinesspartner.
              <fs_citem>-reference2idby_business_partne = ls_input-reference2idbybusinesspartner.
          ENDCASE.
        ENDLOOP.
  ...
  METHOD post_cf.
    ...
    DATA: lt_je_deep                 TYPE TABLE FOR ACTION IMPORT i_journalentrytp~post,
          ls_je_deep                 LIKE LINE OF lt_je_deep,
          lv_cid                     TYPE abp_behv_cid,
          lv_bcy_hkont               TYPE hkont VALUE '9999009999', "拆分凭证补差
          lv_bcy_amountintransaction TYPE zamount_content, "拆分凭证补差金额(交易货币)
          lv_bcy_amountingroup       TYPE zamount_content. "拆分凭证补差金额(集团货币)
    READ TABLE lt_input INTO DATA(ls_input_1) INDEX 1.
    "1.检查该导入凭证号是否存在
    SELECT SINGLE *
            FROM zztfi_0003 WITH PRIVILEGED ACCESS
            WHERE pk_no = @ls_input_1-pk_no
             AND  pk_line_no_1 = @ls_input_1-pk_line_no_1
             AND  flag = 'S'
             INTO @DATA(ls_zztfi_0003_now).
    IF sy-subrc = 0.
      flag = 'S'.
      msg = |导入凭证{ ls_input_1-pk_no }行{ ls_input_1-pk_line_no_1 }已存在对应的SAP会计凭证{ ls_zztfi_0003_now-accountingdocument }|.
      LOOP AT lt_input ASSIGNING FIELD-SYMBOL(<fs_input>).
        <fs_input>-flag = 'S'.
        <fs_input>-msg = msg.
        <fs_input>-accountingdocument = ls_zztfi_0003_now-accountingdocument.
        <fs_input>-fiscalperiod = ls_zztfi_0003_now-fiscalperiod.
      ENDLOOP.
      RETURN.
    ENDIF.
...
*        <fs_entry>-original_reference_document = ls_input_1-pk_no.
        ...
        "行项目
        LOOP AT lt_input INTO DATA(ls_input).
          IF ls_input-debitcreditcode = 'H'.
            lv_bcy_amountintransaction = lv_bcy_amountintransaction - abs( ls_input-amountintransactioncurrency ).
            lv_bcy_amountingroup = lv_bcy_amountingroup - abs( ls_input-amountingroupcurrency ).
          ELSE.
            lv_bcy_amountintransaction = lv_bcy_amountintransaction + abs( ls_input-amountintransactioncurrency ).
            lv_bcy_amountingroup = lv_bcy_amountingroup + abs( ls_input-amountingroupcurrency ).
          ENDIF.
        ...
        ENDLOOP.
        IF lv_bcy_amountintransaction NE 0.
          APPEND INITIAL LINE TO <fs_entry>-item ASSIGNING <fs_item>.
          <fs_item>-glaccount-content = lv_bcy_hkont.
          <fs_item>-amount_in_transaction_currency-currency_code = ls_input_1-transactioncurrency.
          <fs_item>-amount_in_group_currency-currency_code = ls_input_1-transactioncurrency.
          IF lv_bcy_amountintransaction < 0.
            "补借方
            <fs_item>-debit_credit_code = 'S'.
            <fs_item>-amount_in_transaction_currency-content =  abs( lv_bcy_amountintransaction ).
            <fs_item>-amount_in_group_currency-content =  abs( lv_bcy_amountingroup ).
          ELSE.
            "补贷方
            <fs_item>-debit_credit_code = 'H'.
            <fs_item>-amount_in_transaction_currency-content = - abs( lv_bcy_amountintransaction ).
            <fs_item>-amount_in_group_currency-content = - abs( lv_bcy_amountingroup ).
          ENDIF.
        ENDIF.
